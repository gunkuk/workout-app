...
Plan C3 SDD 레저:
C3 Task 1: complete (94d7b42..ffbd7bb, review Approved — validationCore 분리 byte-equivalent, 브라우저 번들 node:fs 0 실증, 기존 .mjs 테스트 21개 무수정)
C3 Task 2: complete (ffbd7bb..583264f, review Approved — 전환=새 InstanceState(같은 프로그램 포함), validate 실패 시 미등록, lint 경계 준수)
C3 Task 3: complete (583264f..6d67dc5, review Approved — 두 함정(stale sessionId·가드 순서) 회피 확인, validateAnchor 하드 게이트, 날짜 고정 테스트)
C3 Task 4: complete (6d67dc5..53bca85, review Approved — Dexie v2 진짜 마이그레이션 테스트, 구백업 하위호환(?? []), manual TM end-to-end fold 검증)
C3 Task 5: complete (53bca85..ddd3b17 + 레이스 수정 9b77204, review Important 1건 → 컨트롤러 즉시 수정: wakeLock release-vs-in-flight-request 누수를 released 플래그로 차단)
C3 Task 6: complete (9b77204..443a7f9, 컨트롤러 검증 — VITE_BASE 오버라이드 실측, workflow_dispatch 전용(자동 push 없음), deploy.md 자기완결)
컨트롤러 최종 브라우저 골든패스(2026-07-10): 온보딩→전역 설치배너+닫기→TM 수동편집(bench 110 반영)→calendar 전환(수요일 startDate 에러 게이트 발화→화요일 통과)→오늘(금)=day4 데드 자동 계산→통증일 옵션·힌지 워밍업 확인→외부 세션 UI(10부위)→콘솔·네트워크 에러 0
=== Plan C3 (stage1-C3-library-deploy) 완료 2026-07-10: 6/6 tasks, 258 tests, 4게이트(vitest/tsc/lint/build) clean ===
=== Stage 1 전체 완료: A(8)+B1(7)+B2(8)+C1(8)+C2(8)+R(6)+C3(6) = 51 tasks, 258 tests ===
잔여(사용자 결정 대기): GitHub repo 생성+push+Pages 활성화(docs/deploy.md 절차) — 원격 배포는 사용자 확인 필요
  → 2026-07-10 사용자 확인으로 실행 완료: github.com/gunkuk/workout-app + Pages 배포(3회: 초기·UI v1·UI v2 Boostcamp 클론)

후속(2026-07-10 저녁): UI v1(다크 기본) 사용자 반려 → 리서치 워크플로(Boostcamp 픽셀샘플링+오픈소스 3종+헬스장 UX) → UI v2 Boostcamp 클론(커밋 6417797+8719292, Playwright VLM 검수 루프) 배포. 테스트 258 무수정 유지.

=== 이월 Minor 최종 판정 (2026-07-10 23:40, Fable 직접 — 상세: decision_tree/actions/2026-07-05-001_stage1-autorun/final-report.md) ===
(1) '골각' 오타: wontfix(히스토리 rewrite 비용>이득) (2) doubleProgression.sets: 의도된 여분 파라미터로 종결(규칙은 마지막 세트만 판정 — 스펙 §2-2) (3) fold-accessory 부분검증: toEqual로 강화·해소 (4) nextCyclePos 다주 분기: 2주 픽스처 테스트 추가·해소 (5) 외부세션-only 주: 도메인 동결 유지·알려진 제약으로 종결.
PACT 최종 보고 작성됨: decision_tree/actions/2026-07-05-001_stage1-autorun/final-report.md (원 지시 재진술+산출 대조+롤백 지점)

=== Fable 최종 seam 리뷰 (2026-07-10 23:45, 컨트롤러 직접) ===
대상: src/screens/today/useTodaySession.ts (R-T5 해체 + UI2 재구성, 두 번 수술된 최고위험 파일) 전문 정독.
...
=== KK 4-day + 진행위치 (2026-07-11 오후) ===
과학 평가: 4렌즈+판정관 워크플로(20판정: 적정7/조정11/부적정2) — 햄둔 볼륨 초과·후면어깨 0세트 교정, T2 판정게이트(경량 마지막→스타터 첫세트) 재설계, 하체 backoff AMRAP 제거, 상체 램프 10%p.
kk-4day.json: 2주 사이클(데드 볼륨↔헤비), 검증기 완화(slotId 전역→주내 유일, 주간 재사용=DP 상태연속+exerciseId 일치 강제), 번들 등록, 시드 테스트 5개.
기반: 운동 6종 추가·USER_PLATES(1.25 제거→5kg 단위)·내장 프로그램 UI (커밋 7e4733f, 오라클 무수정 323).
진행 위치 조정(97bc255): fastForwardTo(빈 완료 세션 — 판정 무영향 확립 패턴), 프로그램 탭 카드, 343 tests.
주의 기록: sessionIdFor가 src/domain/sessionId.ts로 이동(순수함수 이동+re-export shim, lint 경계 필요 — 도메인 로직 무변경, 전 테스트 통과로 무결성 증빙).
