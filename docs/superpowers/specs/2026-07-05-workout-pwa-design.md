...
2. **악세사리** — 요일별 배정(화 랫풀/수 카프/목 리어델트/금 머신이두/토 CSR). 슬롯별 규칙:
   - 더블 프로그레션: 세트 2~3, 목표 rep 범위 **슬롯별** — 랫풀·CSR·머신이두 8~12 / 카프·리어델트 12~20. **마지막 세트 상한 도달 → 다음 세션 +1 스텝, rep 하한 리셋.** RIR 게이트 없음(머신 위주 저부담 구성 — 사용자 확정 2026-07-05, rir 필드는 선택 통계용으로만 유지).
...
3. **TM 자동 증량** —
   ...
   - T1 진리표(`progressionParams`):
     ...
     |---|---|
     | 0~1 | 제안: 동결(기본) / −5kg |
     | 2~3 | 자동(파생): 벤치/OHP +2.5, 스쿼트/데드 +5 |
     | 4+ | 제안: 추가 증량(+5/+10) |
     (전 리프트 nSuns 표준 증량폭 — 데드 +5 사용자 확정 2026-07-05, 진리표는 공식 시트와 완전 일치.)
...
5. **세션 UX** —
   ...
   - 운동 스킵/대체 + **데드 통증일 안전 프리셋 = 경량 컨벤셔널**(같은 동작, TM 50~60%, AMRAP 제거 — 사용자 확정 2026-07-05. RDL은 제외: 절대 안 함·원전에도 없음). 대체 세트는 이력·e1RM 분리 표시(§2-6)·TM 판정 제외.
   - 진행 중 세션 복원(즉시 커밋).
...
## 8. 미결정 → 판정 현황 (D5, 2026-07-05)
1. ✅ 악세사리: **슬롯별 rep 범위(8~12 / 카프·리어델트 12~20), RIR 게이트 없음.**
2. ✅ 유효 세트 규칙: **v4.1 기본값 승인.**
3. ✅ %테이블: **공식 스프레드시트 정본** (착수 태스크에서 확정, 볼트 표와 차이 시 공식 우선).
4. ✅ 데드 증량: **+5 표준** (보수 +2.5 폐기 — "2개 할 수 있으면 5키로").
5. ✅ T2 규칙 **승인**: 완수→+스텝(인클라인·CGBP +2.5 / 프론트 +5 / 스모 +2.5) / 2주 연속 미완수→디로드 제안.
6. ✅ 데드 통증일 프리셋: **(a) 경량 컨벤셔널**(TM 50~60%, AMRAP 제거). RDL 영구 제외.

**→ §8 전건 판정 완료 (D5 ✅, 2026-07-05). 스펙 확정 — 구현 계획 단계로.**
