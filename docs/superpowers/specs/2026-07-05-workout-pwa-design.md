...
5. **세션 UX** —
   ...
   - 운동 스킵/대체 + **데드 통증일 안전 프리셋** — RDL은 **사용자 확정으로 제외**(절대 안 함, 원전에도 없음). 프리셋 확정 대기 [§8-6]: (a) 경량 컨벤셔널(TM 50~60%, AMRAP 제거) ★ / (b) 스모 캡(≤85%, AMRAP 제거) / (c) 스킵. 대체 세트는 이력·e1RM 분리 표시(§2-6)·TM 판정 제외.
   - 진행 중 세션 복원(즉시 커밋).
...
5. 🟡 T2 규칙(완수→+스텝 / 인클라인·CGBP +2.5·프론트 +5·스모 +2.5 / 2주 실패 디로드) — 원전 대비 설명 제공됨, 승인 대기.
6. 🟡 데드 통증일 프리셋 — RDL 제외 확정. (a) 경량 컨벤셔널 ★ / (b) 스모 캡 / (c) 스킵 중 선택 대기.
